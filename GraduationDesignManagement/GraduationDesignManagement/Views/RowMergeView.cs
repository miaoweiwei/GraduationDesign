...
namespace SumscopeAddIn.Views
{
    ...
    public partial class RowMergeView : DataGridView
    {
                ...
                else
                {
                    ...
                    if (e.RowIndex == -1)
                    {
                        if (SpanRows.ContainsKey(e.ColumnIndex)) //被合并的列
                        {
                            ...
                            //Color cl = Color.FromArgb(255, 255, 0, 0);

                            //Brushes.Yellow
                            g.DrawString(e.Value + "", e.CellStyle.Font, new SolidBrush(_mergecolumnheaderFontcolor),
                            ...
                            g.DrawString(SpanRows[e.ColumnIndex].Text, e.CellStyle.Font, new SolidBrush(_mergecolumnheaderFontcolor),
        ...
        private void DrawCell(DataGridViewCellPaintingEventArgs e)
        {
            try
            {
                ...
                if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
                {
                    ...
                    string curSelected = this.CurrentRow.Cells[e.ColumnIndex].Value == null
                        ? ""
                        : this.CurrentRow.Cells[e.ColumnIndex].Value.ToString().Trim();
                    if (!string.IsNullOrEmpty(curValue))
                    {
                        ...
                        for (int i = e.RowIndex; i < this.Rows.Count; i++)
                        {
                            if (this.Rows[i].Cells[e.ColumnIndex].Value.ToString().Equals(curValue))
                            {
                                ...
                                if (e.RowIndex != i)
                                {
                                    cellwidth = cellwidth < this.Rows[i].Cells[e.ColumnIndex].Size.Width
                                        ? cellwidth
                                        : this.Rows[i].Cells[e.ColumnIndex].Size.Width;
                                }
                            }
                            else
                            {
        ...
        /// 二维表头的字体颜色
        /// </summary>
        [Description("二维表头字体颜色"), Browsable(true), Category("二维表头")]
        public Color MergeColumnHeaderFontColor
        {
            get { return _mergecolumnheaderFontcolor; }
            set { _mergecolumnheaderFontcolor = value; }
        }
        private Color _mergecolumnheaderFontcolor = Color.Black;
